...
RSpec.describe Hyrax::Migrator::Jobs::VerifyWorkJob, type: :job do
  ...
  context 'when there is a job to run' do
    ...
    before do
      allow(Hyrax::Migrator::Services::VerifyWorkService).to receive(:new).and_return(service)
      allow(service).to receive(:run)
    end

    it 'instantiates and calls the service' do
      expect(Hyrax::Migrator::Services::VerifyWorkService).to receive(:new)
