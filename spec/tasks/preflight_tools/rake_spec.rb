...
RSpec.describe 'preflight_tools rake tasks' do
  ...
  describe 'preflight_tools:preflight' do
    ...
    before do
      load_rake_environment [File.expand_path('../../../lib/tasks/preflight_tools/preflight.rake', __dir__), File.expand_path('../../../lib/hyrax/migrator/preflight_services.rb', __dir__)]
      ENV['work_dir'] = path
      ENV['pidlist'] = 'pidlist.txt'
      allow(Hyrax::Migrator::PreflightServices).to receive(:new).and_return(service)
